#!/usr/bin/env bash
# Build bat into ./out/bat as a static RV64GC musl ELF via cargo-zigbuild —
...
cd .src
cargo zigbuild --release --target riscv64gc-unknown-linux-musl --bin bat
cd ..
mkdir -p out
cp .src/target/riscv64gc-unknown-linux-musl/release/bat out/bat
file out/bat
