#!/usr/bin/env bash
# Build ast-grep into ./out/ast-grep as a static RV64GC musl ELF via cargo-zigbuild —
...
cd ..
mkdir -p out
cp .src/target/riscv64gc-unknown-linux-musl/release/ast-grep out/ast-grep
file out/ast-grep
