#!/usr/bin/env bash
# Build typst into ./out/typst as a static RV64GC musl ELF via cargo-zigbuild —
...
cd .src
cargo zigbuild --release --target riscv64gc-unknown-linux-musl -p typst-cli --bin typst
cd ..
mkdir -p out
cp .src/target/riscv64gc-unknown-linux-musl/release/typst out/typst
file out/typst
